...
export default function Cart() {
      ...
      .then((res) => {
        ...
        const productRequests = items.map((item) =>
          axios
            ...
            .then((productRes) => ({
              ...productRes.data,
              quantity: 1, // Use the quantity from the cart API
              selected: false,
              isDelete: false,
            }))
        );
  ...
  useEffect(() => {
    const total = cartItems.reduce((acc, item) => {
      if (!item.isDelete) return acc + (item.price || 0) * (item.quantity || 0); // Safely access price and quantity
      return acc;
    }, 0);
  ...
  return (
    <>
      <div className="p-4 my-10 lg:mt-20 xl:w-[60%] md:w-[80%] w-full m-auto max-w-[1500px] min-w-[1100px] overflow-x-auto flex flex-col gap-6">
